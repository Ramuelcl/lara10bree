<div class="py- m-6">

    <div class="py- m-6">

        <div class="overflow-x-auto sm:-mx-6 lg:-mx-8">
            <div class="py-2 align-middle inline-block min-w-full sm:px-6 lg:px-8">
                <div class="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">

                    <table class="min-w-full divide-y divide-gray-200">
                        <div class="flex justify-between">
                            <input wire:model.live="search" name="search" type="search"
                                placeholder="ingrese dato a buscar"
                                class="w-1/5 rounded-t-md text-gray-800 dark:text-gray-100 bg-gray-100 dark:bg-gray-800">
                            <div class="m-auto p-2">
                                <label for="is_actives"
                                    class="mx-4 text-gray-800 dark:text-gray-100 bg-gray-100 dark:bg-gray-800">Activos
                                    ?</label>
                                <input type="checkbox" wire:model.live="is_actives" name="is_actives" class="">
                            </div>
                        </div>
                        <thead class="bg-gray-50 dark:bg-gray-600 dark:text-gray-200">
                            <tr>
                                <th scope="col"
                                    class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-200 tracking-wider">
                                    Id</th>
                                <th scope="col"
                                    class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-200 tracking-wider">
                                    Name</th>
                                <th scope="col"
                                    class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-200 tracking-wider">
                                    @Mail</th>
                                <th scope="col"
                                    class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-200 tracking-wider">
                                    Active?</th>
                                <th scope="col"
                                    class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-200 tracking-wider">
                                    Photo</th>
                                <th scope="col" class="flex flex-row px-6 py-3 justify-end">
                                    <div class="m-2">
                                        @hasanyrole('Super-Admin|admin|writer')
                                            <a href="{{ route('admin.users.create') }}"
                                                class="mx-1 p-2 bg-blue-400 rounded">
                                                {{ __('Create') }}
                                            </a>
                                        @endhasanyrole
                                    </div>
                                </th>
                            </tr>
                        </thead>

                        <tbody class="bg-gray-100 dark:bg-gray-700 divide-y divide-gray-200">
                            @if ($datas->count())
                                @foreach ($datas as $data)
                                    <tr
                                        class="@if ($loop->even) even:bg-gray-100 dark:even:bg-gray-800 @else odd:bg-gray-200 dark:odd:bg-gray-900 @endif">
                                        <td class="px-6 py-4 whitespace-nowrap">{{ $data->id }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap">{{ $data->name }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap">{{ $data->email }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap">{{ $data->is_active ? 'Si' : 'No' }}
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            @if (Storage::exists($data->profile_photo_path))
                                                <img src="{{ Storage::url($data->profile_photo_path) }}" class="w-5">
                                            @else
                                                <img src="{{ Storage::url('public/images/avatars/default.png') }}"
                                                    class="w-5">
                                            @endif
                                        </td>
                                        <td class="px-6 py-4 text-right text-sm">
                                            @hasanyrole('Super-Admin|admin|editor')
                                                <a href="{{ route('admin.users.edit', $data->id) }}"
                                                    class="m-2 p-2 bg-green-400 rounded">{{ __('Edit') }}</a>
                                            @endhasanyrole
                                            @hasanyrole('super-admin|admin')
                                                <a href="{{ route('admin.users.destroy', $data->id) }}"
                                                    class="m-2 p-2 bg-red-400 rounded">{{ __('Delete') }}</a>
                                            @endhasanyrole
                                        </td>
                                    </tr>
                                @endforeach
                            @else
                                <strong>{{ __('not records') }}</strong>
                                <!-- More items... -->
                        </tbody>

                        @endif
                    </table>
                    <div class="flex justify-between">
                        <select wire:model.live="pag"
                            class="mx-auto h-10 my-4 rounded-md text-gray-800 dark:text-gray-100 bg-gray-100 dark:bg-gray-800">
                            <option value="" disabled>Seleccione</option>
                            @foreach ($pags as $pag)
                                <option value="{{ $pag }}"
                                    class="dark:text-gray-100 bg-gray-100 dark:bg-gray-800">{{ $pag }}
                                </option>
                            @endforeach
                        </select>
                        <div class="m-2 p-2">{{ $datas->links('pagination::tailwind') }}
                        </div>
                    </div>

                </div>
            </div>
        </div>

    </div>
</div>
